...
-	***Rama***: es un **"clon"** de los archivos que tenemos en nuestro repositorio, sobre los que podemos hacer cambios sin afectar a los originales, que pertenecen al *"tronco"*, que se denomina **"Master"**. Se pueden hacer cuantas ramas se quiera, y sobre ellas se puede "experimentar" sin afectar al *Master*. Si queremos que las operaciones que hemos realizado sobre esta rama se incorporen al *Master* se puede fusionar esta *rama* con él mediante un **pull request**.
...
En el siguiente enlace se puede acceder al GitHub de Nikolai Shurupov, donde se puede observar que hay varios repositorios, uno resultado de realizar un *Fork* sobre el repositorio del GitHub del  **CLS**(Citation Style Language). En el repositorio de **"RMarkdown"** se puede encontrar el script utilizado para esta práctica. Para la elaboración del Script de la práctica se han hecho diversas modificaciones del archivo en el GitHub mediante push and pull desde R studio, por lo que se pueden ver los cambios hechos a lo largo de la realización de la práctica.
