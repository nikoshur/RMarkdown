...
-	***Rama***: es un **"clon"** de los archivos que tenemos en nuestro repositorio, sobre los que podemos hacer cambios sin afectar a los originales, que pertenecen al *"tronco"*, que se denomina **"Master"**. Se pueden hacer cuantas ramas se quiera, y sobre ellas se pueden hacer se pùede "experimentar" sin afectar al *Master*. Si queremos que las operaciones que hemos realizado sobre esta rama se incorporen al *Master* se puede fusionar estar *rama* con el mediante un **pull request**.
...
-	***Fork***: es una clonación de un repositorio público de **otra cuenta a nuestra cuenta**, de manera que podemos trabajar sobre los repositorios de otra gente, alojándolos en nuestra propia cuenta y sin alterar el reposotorio original.

En el siguiente enlace se puede acceder al GitHub de Nikolai Shurupov, donde se puede observar que hay varios repositorios, uno resultado de realizar un *Fork* sobre el repositorio del GitHub del  **CLS**(Citation Style Language). En el repositorio de **"RMarkdown"** se puede encontrar el script utilizado para esta práctica.
